...
- **Аудио** — плеер, транскрипт (faster-whisper), `05data/aviora_audio_transcripts/`; см. `docs/AUDIO.md`
- **Главный файл ★** — после Scan группы дубликатов по тексту; в поиске бейдж «★ главный» / «копия (N)»; лента **Сервис → Главный** или ПКМ по строке поиска; реестр `05data/aviora_canonical.json`
...
- **Чат LM Studio** — модель из `/v1/models`; в контексте ★ = рекомендуемый главный среди копий
- **Поиск** — Enter после Scan; главные выше в списке
